...
import { db } from "../lib/firebase"; // Yolun lib olduğuna emin ol
import { doc, onSnapshot, collection, addDoc } from "firebase/firestore";
import { Share2, CreditCard, ShoppingBag, PackageCheck, Copy, X } from "lucide-react";
...
export default function Home() {
  ...
  const [odemeYontemi, setOdemeYontemi] = useState(null); // 'iban' veya 'kredi_karti'
  ...
  const bildirimGonder = async (e) => {
    ...
    try {
        ...
        alert("✅ Bağış bildiriminiz alındı! Kontrol edildikten sonra size SMS/WhatsApp ile dönüş yapılacaktır.");
        setModalAcik(false);
    ...
    <div className="min-h-screen bg-[#ebe7e0] flex items-center justify-center p-0 md:p-4 font-sans text-gray-800">
      ...
      <div className="w-full max-w-[1000px] bg-white shadow-2xl md:rounded-3xl overflow-hidden flex flex-col md:flex-row min-h-screen md:min-h-[650px] border border-white/50">
        ...
        <div className="w-full md:w-7/12 p-6 md:p-8 bg-white flex flex-col relative">
          ...
          {/* BUTONLAR - GÜNCELLENDİ: MODAL AÇIYOR */}
                ...
                <div className="p-6">
                    ...
                    <div className="grid grid-cols-2 gap-3 mb-6">
                        <button 
                            ...
                            className={`p-4 rounded-xl border-2 font-bold flex flex-col items-center gap-2 transition ${odemeYontemi === 'iban' ? 'border-amber-500 bg-amber-50 text-amber-700' : 'border-gray-100 hover:border-gray-300 text-gray-500'}`}
                        >
                        ...
                        <button 
                            ...
                            className={`p-4 rounded-xl border-2 font-bold flex flex-col items-center gap-2 transition ${odemeYontemi === 'kredi_karti' ? 'border-amber-500 bg-amber-50 text-amber-700' : 'border-gray-100 hover:border-gray-300 text-gray-500'}`}
                        >
                            <span className="text-2xl">💳</span>
                            KREDİ KARTI
                        </button>
                    ...
                    {/* 2. IBAN BİLGİLERİ VE FORM */}
                    {odemeYontemi === 'iban' && (
                        <div className="space-y-5 animate-in fade-in">
                            {/* Hesap Bilgileri */}
                            <div className="bg-gray-50 p-4 rounded-xl border border-gray-200 text-sm space-y-3 relative">
                                <div>
                                    <span className="text-xs font-bold text-gray-400 block">BANKA</span>
                                    <span className="font-bold text-gray-800">{data?.bankaAdi || "Banka Girilmedi"}</span>
                                </div>
                                <div>
                                    <span className="text-xs font-bold text-gray-400 block">ALICI ADI</span>
                                    <div className="flex justify-between items-center">
                                        <span className="font-bold text-gray-800">{data?.aliciAdi || "Alıcı Girilmedi"}</span>
                                        <button onClick={() => navigator.clipboard.writeText(data?.aliciAdi)} className="text-amber-600 text-xs font-bold flex items-center gap-1 hover:underline"><Copy size={12}/> Kopyala</button>
                                    </div>
                                </div>
                                <div className="bg-white p-2 rounded border border-gray-200">
                                    <span className="text-xs font-bold text-gray-400 block mb-1">IBAN NO</span>
                                    <div className="flex justify-between items-center gap-2">
                                        <span className="font-mono font-bold text-gray-800 text-xs sm:text-sm break-all">{data?.iban || "TR..."}</span>
                                        <button onClick={() => navigator.clipboard.writeText(data?.iban)} className="text-amber-600 text-xs font-bold flex items-center gap-1 hover:underline shrink-0"><Copy size={12}/> Kopyala</button>
                                    </div>
                                </div>
                                <div className="text-xs text-amber-700 bg-amber-50 p-2 rounded">
                                    <strong>NOT:</strong> Açıklama kısmına "BAĞIŞ" yazmayı unutmayınız.
                                </div>
                            </div>
